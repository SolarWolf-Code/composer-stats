...
def calculate_tracking_error(live_returns: List[float], backtest_returns: List[float]) -> float:
    """
...
def calculate_correlation(live_returns: List[float], backtest_returns: List[float]) -> float:
    """
    ...
    numerator = sum((lr - mean_live) * (br - mean_backtest) 
...
def calculate_max_deviation(live_values: List[float], backtest_values: List[float]) -> float:
    """
...
def calculate_mean_deviation(live_values: List[float], backtest_values: List[float]) -> float:
    """
...
def compute_deviation_metrics(
    ...
    - risk_score: A composite risk score (0-100, higher = more risk)
    """
    ...
    cumulative_return_dev = calculate_cumulative_return_deviation(aligned_live, aligned_backtest)
    
    # Calculate a composite risk score (0-100)
    # Higher tracking error, lower correlation, and higher deviations = higher risk
    risk_score = 0.0
    
    # Tracking error component (0-40 points): >20% tracking error = 40 points
    risk_score += min(tracking_error / 0.20 * 40, 40)
    
    # Correlation component (0-30 points): correlation < 0.7 adds points
    correlation_penalty = max(0, (0.9 - correlation)) / 0.9 * 30
    risk_score += correlation_penalty
    
    # Mean deviation component (0-30 points): >10% mean deviation = 30 points
    risk_score += min(mean_deviation / 0.10 * 30, 30)
